...
- `EMAIL_CODES_TO_CONSOLE=1` forces that same console path even when Supabase *is* configured, which sidesteps the built-in sender's hourly ceiling during a demo. Anyone able to read the logs can then sign in as anyone, so leave it unset in normal operation.
